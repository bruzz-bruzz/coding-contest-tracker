import './App.css'
type Data = {
    repo:string
}
export default function Github({repo}:Data){
    return (
        <div className='p-4 text-center text-gray-500'>
            <h1>Made by bruzz-bruzz</h1>
            <h3><a href='https://github.com/bruzz-bruzz' className='underline' target="_blank">GitHub Profile</a></h3>
            <h3><a href={`${repo}`} className='underline' target="_blank">GitHub Repository</a></h3>
            <h3><a className='underline' target='_blank' href="https://github.com/bruzz-bruzz/coding-contest-tracker/blob/main/PRIVACY.md">Privacy Policy</a></h3>
            <h3><a className='underline' target='_blank' href="https://github.com/bruzz-bruzz/coding-contest-tracker/blob/main/TOS.md">Terms of Service</a></h3>
            <h3><a className='underline' target='_blank' href="https://coding-contest-tracker-one.vercel.app/discordBot">Discord Bot</a></h3>
        </div>
    )
}